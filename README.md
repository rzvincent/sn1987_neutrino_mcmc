...
* **`flux_osc_IS.ipynb` & `flux_osc_TPI.ipynb`**: Plots integrated flux curves for the Inverse Seesaw and Type-I Seesaw models. *(Note: Verify that `Seesaw_Inverse` is set to `True` or `False` in `definition.py` before running).*
...
* **`stacked_*.ipynb`**: Plots 2-parameter contours, stacking the constraints from the Vartanyan23 model with 22 independent mass spectra constraints.
